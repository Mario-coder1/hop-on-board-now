...
-- Add Stripe fee settings and update payout trigger to account for them
INSERT INTO public.platform_settings (key, value) VALUES ('stripe_fee_percent', 1.5)
ON CONFLICT (key) DO NOTHING;
INSERT INTO public.platform_settings (key, value) VALUES ('stripe_fee_fixed_cents', 25)
ON CONFLICT (key) DO NOTHING;
...
BEGIN
  ...
  -- amount_paid = (base + commission) + stripe_fee   where stripe_fee = amount_paid * pct/100 + fixed
  -- => net (after Stripe) = amount_paid - stripe_fee = base * (1 + commission_pct/100)
  ...
  VALUES (v_wallet_id, 'driver_payout', v_payout, v_commission + v_stripe_fee, NEW.ride_id,
          'Výplata za jazdu (platforma ' || v_commission_pct || ' % + Stripe ' || v_stripe_fee || ' € pripočítané k cene pasažiera)');
